...
#include "Misc/BoneRigComponentSequenceEditorHelpers.h"
...
const FName FBoneRigComponentSequenceEditorToolkit::SequencerMainTabId(TEXT("Sequencer_SequencerMain"));
...
static TArray<FBoneRigComponentSequenceEditorToolkit*> OpenToolkits;

void FBoneRigComponentSequenceEditorToolkit::IterateOpenToolkits(TFunctionRef<bool(FBoneRigComponentSequenceEditorToolkit&)> Iter)
{
	for (FBoneRigComponentSequenceEditorToolkit* Toolkit : OpenToolkits)
	{
		if (!Iter(*Toolkit))
		{
			return;
...
FBoneRigComponentSequenceEditorToolkit::FBoneRigComponentSequenceEditorToolkitOpened& FBoneRigComponentSequenceEditorToolkit::OnOpened()
{
	static FBoneRigComponentSequenceEditorToolkitOpened OnOpenedEvent;
	return OnOpenedEvent;
}

/* FBoneRigComponentSequenceEditorToolkit structors
 *****************************************************************************/

FBoneRigComponentSequenceEditorToolkit::FBoneRigComponentSequenceEditorToolkit(const TSharedRef<ISlateStyle>& InStyle)
	...
	, Style(InStyle)
{
	...
	int32 NewIndex = SequencerModule.GetAddTrackMenuExtensibilityManager()->GetExtenderDelegates().Add(
		FAssetEditorExtender::CreateRaw(this, &FBoneRigComponentSequenceEditorToolkit::HandleMenuExtensibilityGetExtender));
...
FBoneRigComponentSequenceEditorToolkit::~FBoneRigComponentSequenceEditorToolkit()
{
...
/* FBoneRigComponentSequenceEditorToolkit interface
 *****************************************************************************/

void FBoneRigComponentSequenceEditorToolkit::Initialize(const EToolkitMode::Type Mode, const TSharedPtr<IToolkitHost>& InitToolkitHost, ULevelSequence* InLevelSequence)
{
	// create tab layout
	...
	PlaybackContext = MakeShared<FBoneRigComponentSequencePlaybackContext>();
	...
	FSequencerInitParams SequencerInitParams;
	{
		...
		SequencerInitParams.EventContexts.Bind(PlaybackContext.ToSharedRef(), &FBoneRigComponentSequencePlaybackContext::GetEventContexts);
		SequencerInitParams.PlaybackContext.Bind(PlaybackContext.ToSharedRef(), &FBoneRigComponentSequencePlaybackContext::GetAsObject);

		SequencerInitParams.ViewParams.UniqueName = "LevelSequenceEditor";
		SequencerInitParams.ViewParams.ScrubberStyle = ESequencerScrubberStyle::FrameBlock;
		SequencerInitParams.ViewParams.OnReceivedFocus.BindRaw(this, &FBoneRigComponentSequenceEditorToolkit::OnSequencerReceivedFocus);

		SequencerInitParams.HostCapabilities.bSupportsCurveEditor = true;
		SequencerInitParams.HostCapabilities.bSupportsSaveMovieSceneAsset = true;

		TSharedRef<FExtender> ToolbarExtender = MakeShared<FExtender>();
		ToolbarExtender->AddToolBarExtension("Base Commands", EExtensionHook::Before, nullptr, FToolBarExtensionDelegate::CreateSP(this, &FBoneRigComponentSequenceEditorToolkit::ExtendSequencerToolbar));
		SequencerInitParams.ViewParams.ToolbarExtender = ToolbarExtender;
	}

	Sequencer = FModuleManager::LoadModuleChecked<ISequencerModule>("Sequencer").CreateSequencer(SequencerInitParams);
	SpawnRegister->SetSequencer(Sequencer);
	Sequencer->OnActorAddedToSequencer().AddSP(this, &FBoneRigComponentSequenceEditorToolkit::HandleActorAddedToSequencer);
	...
	LevelEditorModule.OnMapChanged().AddRaw(this, &FBoneRigComponentSequenceEditorToolkit::HandleMapChanged);
	...
	LevelSequenceEditorModule.OnMasterSequenceCreated().AddRaw(this, &FBoneRigComponentSequenceEditorToolkit::HandleMasterSequenceCreated);

	FBoneRigComponentSequenceEditorToolkit::OnOpened().Broadcast(*this);

	{
		...
		if (VRMode != nullptr)
		{
			VRMode->OnVREditingModeExit_Handler.BindSP(this, &FBoneRigComponentSequenceEditorToolkit::HandleVREditorModeExit);
...
FText FBoneRigComponentSequenceEditorToolkit::GetBaseToolkitName() const
{
	return LOCTEXT("AppLabel", "Level Sequence Editor");
}


FName FBoneRigComponentSequenceEditorToolkit::GetToolkitFName() const
{
...
FLinearColor FBoneRigComponentSequenceEditorToolkit::GetWorldCentricTabColorScale() const
{
	return FLinearColor(0.7, 0.0f, 0.0f, 0.5f);
}


FString FBoneRigComponentSequenceEditorToolkit::GetWorldCentricTabPrefix() const
{
	return LOCTEXT("WorldCentricTabPrefix", "Sequencer ").ToString();
}


void FBoneRigComponentSequenceEditorToolkit::RegisterTabSpawners(const TSharedRef<class FTabManager>& InTabManager)
{
	...
	InTabManager->RegisterTabSpawner(SequencerMainTabId, FOnSpawnTab::CreateSP(this, &FBoneRigComponentSequenceEditorToolkit::HandleTabManagerSpawnTab))
...
void FBoneRigComponentSequenceEditorToolkit::UnregisterTabSpawners(const TSharedRef<class FTabManager>& InTabManager)
{
	if (!IsWorldCentricAssetEditor())
	{
...
/* FBoneRigComponentSequenceEditorToolkit implementation
 *****************************************************************************/

void FBoneRigComponentSequenceEditorToolkit::ExtendSequencerToolbar(FToolBarBuilder& ToolbarBuilder)
{
	ToolbarBuilder.AddWidget(PlaybackContext->BuildWorldPickerCombo());
}

void FBoneRigComponentSequenceEditorToolkit::AddDefaultTracksForActor(AActor& Actor, const FGuid Binding)
{
	...
	// add default tracks
	for (const FBoneRigComponentSequenceTrackSettings& TrackSettings : GetDefault<UBoneRigComponentSequenceEditorSettings>()->TrackSettings)
	{
		...
		for (const FSoftClassPath& DefaultTrack : TrackSettings.DefaultTracks)
		{
			...
			for (const FBoneRigComponentSequenceTrackSettings& ExcludeTrackSettings : GetDefault<UBoneRigComponentSequenceEditorSettings>()->TrackSettings)
			{
		...
		for (const FBoneRigComponentSequenceTrackSettings& ExcludeTrackSettings : GetDefault<UBoneRigComponentSequenceEditorSettings>()->TrackSettings)
		{
			...
			for (const FBoneRigComponentSequencePropertyTrackSettings& PropertyTrackSettings : ExcludeTrackSettings.ExcludeDefaultPropertyTracks)
			{
		...
		for (const FBoneRigComponentSequencePropertyTrackSettings& PropertyTrackSettings : TrackSettings.DefaultPropertyTracks)
		{
...
/* FBoneRigComponentSequenceEditorToolkit callbacks
 *****************************************************************************/

void FBoneRigComponentSequenceEditorToolkit::OnSequencerReceivedFocus()
{
...
void FBoneRigComponentSequenceEditorToolkit::HandleAddComponentActionExecute(UActorComponent* Component)
{
	const FScopedTransaction Transaction(LOCTEXT("AddComponent", "Add Component"));

	FString ComponentName = Component->GetName();

	if (Component->IsA(UBoneRigComponent::StaticClass())) {
		if (AActor* Actor = Component->GetOwner()) {
			TArray<UActorComponent*> Arr = Actor->GetComponentsByClass(USpineRigBoneDriverComponent::StaticClass());

			for (UActorComponent* TestComp : Arr) {
				if (USpineRigBoneDriverComponent* Casted = Cast<USpineRigBoneDriverComponent>(TestComp)) {
					HandleAddComponentActionExecute(TestComp);
				}
			}
		}
	}
...
void FBoneRigComponentSequenceEditorToolkit::HandleActorAddedToSequencer(AActor* Actor, const FGuid Binding)
{
	AddDefaultTracksForActor(*Actor, Binding);
}


void FBoneRigComponentSequenceEditorToolkit::HandleVREditorModeExit()
{
	UWorld* World = PlaybackContext->Get();
...
void FBoneRigComponentSequenceEditorToolkit::HandleMapChanged(class UWorld* NewWorld, EMapChangeType MapChangeType)
{
...
void FBoneRigComponentSequenceEditorToolkit::AddShot(UMovieSceneCinematicShotTrack* ShotTrack, const FString& ShotAssetName, const FString& ShotPackagePath, FFrameNumber ShotStartTime, FFrameNumber ShotEndTime, UObject* AssetToDuplicate, const FString& FirstShotAssetName)
{
	// Create a level sequence asset for the shot
	UObject* ShotAsset = BoneRigComponentSequenceEditorHelpers::CreateLevelSequenceAsset(ShotAssetName, ShotPackagePath, AssetToDuplicate);
	...
	const UBoneRigComponentSequenceMasterSequenceSettings* MasterSequenceSettings = GetDefault<UBoneRigComponentSequenceMasterSequenceSettings>();
	...
	if (MasterSequenceSettings->SubSequenceNames.Num())
	{
		...
		for (auto SubSequenceName : MasterSequenceSettings->SubSequenceNames)
		{
			...
			{
				UObject* SubSequenceAsset = BoneRigComponentSequenceEditorHelpers::CreateLevelSequenceAsset(SubSequenceAssetName, ShotPackagePath);
...
void FBoneRigComponentSequenceEditorToolkit::HandleMasterSequenceCreated(UObject* MasterSequenceAsset)
{
...
TSharedRef<FExtender> FBoneRigComponentSequenceEditorToolkit::HandleMenuExtensibilityGetExtender(const TSharedRef<FUICommandList> CommandList, const TArray<UObject*> ContextSensitiveObjects)
{
	...
	AddTrackMenuExtender->AddMenuExtension(
		...
		CommandList,
		FMenuExtensionDelegate::CreateRaw(this, &FBoneRigComponentSequenceEditorToolkit::HandleTrackMenuExtensionAddTrack, ContextSensitiveObjects));

	return AddTrackMenuExtender;
}


TSharedRef<SDockTab> FBoneRigComponentSequenceEditorToolkit::HandleTabManagerSpawnTab(const FSpawnTabArgs& Args)
{
...
void FBoneRigComponentSequenceEditorToolkit::HandleTrackMenuExtensionAddTrack(FMenuBuilder& AddTrackMenuBuilder, TArray<UObject*> ContextObjects)
{
	...
	if (Actor != nullptr)
	{
		AddTrackMenuBuilder.BeginSection("Components", LOCTEXT("ComponentsSection", "Components"));
		{
			for (UActorComponent* Component : Actor->GetComponents())
			{
				if (Component)
				{
					FUIAction AddComponentAction(FExecuteAction::CreateSP(this, &FBoneRigComponentSequenceEditorToolkit::HandleAddComponentActionExecute, Component));
		...
		if (USkeletalMeshComponent* SkeletalComponent = Cast<USkeletalMeshComponent>(ContextObjects[0]))
		{
			...
			AddTrackMenuBuilder.BeginSection("Anim Instance", AnimInstanceLabel);
			{
				AddTrackMenuBuilder.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateSP(this, &FBoneRigComponentSequenceEditorToolkit::BindAnimationInstance, SkeletalComponent)
					)
				);
			}
			AddTrackMenuBuilder.EndSection();
...
void FBoneRigComponentSequenceEditorToolkit::BindAnimationInstance(USkeletalMeshComponent* SkeletalComponent)
{
	UAnimInstance* AnimInstance = SkeletalComponent->GetAnimInstance();
...
bool FBoneRigComponentSequenceEditorToolkit::OnRequestClose()
{
	UWorld* World = PlaybackContext->Get();
	UVREditorMode* VRMode = Cast<UVREditorMode>(GEditor->GetEditorWorldExtensionsManager()->GetEditorWorldExtensions(World)->FindExtension(UVREditorMode::StaticClass()));
	if (VRMode != nullptr)
...
bool FBoneRigComponentSequenceEditorToolkit::CanFindInContentBrowser() const
{
